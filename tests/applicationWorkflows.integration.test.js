...
test('storefront catalog scope honors query and header navigation and never exposes hidden products', async () => {
  ...
  await Store.updateOne({ _id: seller.store.id }, { status: 'PUBLISHED' });
  const sellerCategory = await Category.create({ storeId: seller.store.id, name: 'Accessories', slug: 'accessories' });
  const sellerProduct = await createProduct({ storeId: seller.store.id, category: sellerCategory._id, name: 'Scoped Workflow Saree', sizes: ['S', 'M'], colors: ['Rose'], fabric: 'Silk', occasion: 'Festive', showInTrending: true, isBestSeller: true });
  const secondSellerProduct = await createProduct({ storeId: seller.store.id, category: sellerCategory._id, name: 'Pearl Accessories Set', sizes: ['XL'], colors: ['Ivory'], fabric: 'Cotton', occasion: 'Casual', price: 600, rating: 4.8 });
  ...
  assert.deepEqual(new Set(queryResult.map(p => p._id)), new Set([String(sellerProduct._id), String(secondSellerProduct._id)]));
  assert.deepEqual(new Set(headerResult.map(p => p._id)), new Set([String(sellerProduct._id), String(secondSellerProduct._id)]));
  const paged = await get(`/api/products?store=${seller.store.slug}&page=1&limit=1&includeFacets=true`);
  assert.equal(paged.items.length, 1);
  assert.equal(paged.total, 2);
  assert.equal(paged.totalPages, 2);
  assert.equal(paged.facets.categories.find(item => item.value === String(sellerCategory._id)).count, 2);
  assert.equal(paged.facets.sizes.find(item => item.value === 'XL').count, 1);
  assert.equal(paged.facets.colors.find(item => item.value === 'Rose').count, 1);
  assert.deepEqual((await get(`/api/products?store=${seller.store.slug}&size=M,XL&page=1&limit=24`)).items.map(item => item._id).sort(), [String(sellerProduct._id), String(secondSellerProduct._id)].sort());
  assert.deepEqual((await get(`/api/products?store=${seller.store.slug}&trending=true&page=1&limit=24`)).items.map(item => item._id), [String(sellerProduct._id)]);
  assert.deepEqual(new Set((await get(`/api/products?store=${seller.store.slug}&search=accessory&page=1&limit=24`)).items.map(item => item._id)), new Set([String(sellerProduct._id), String(secondSellerProduct._id)]));
  assert.deepEqual((await get(`/api/products?store=${seller.store.slug}&search=pearl&page=1&limit=24`)).items.map(item => item._id), [String(secondSellerProduct._id)]);
