...
router.get('/', asyncHandler(async (req, res) => {
  ...
  res.json(await User.find(filter).select('-password').sort('-createdAt').skip(skip).limit(limit).lean());
}));
...
router.patch('/:userId/block', validateObjectIdParam('userId'), asyncHandler(async (req, res) => {
  ...
  const customer = await User.findByIdAndUpdate(req.params.userId, { isBlocked }, { new: true }).select('-password');
  if (!customer) throw notFound('Customer not found');
  res.json(customer);
}));

router.patch('/:userId/promote-admin', validateObjectIdParam('userId'), asyncHandler(async (req, res) => {
  ...
  await user.save();
  logAudit({ req, action: 'ROLE_PROMOTE', entityType: 'User', entityId: user._id, after: { role: 'admin' } });
  res.json(user);
}));

router.patch('/:userId/demote-admin', validateObjectIdParam('userId'), asyncHandler(async (req, res) => {
  ...
  user.activeMode = 'customer';
  await user.save();
  logAudit({ req, action: 'ROLE_DEMOTE', entityType: 'User', entityId: user._id, after: { role: 'customer' } });
  res.json(user);
}));
