...
const storeSchema = new mongoose.Schema({
  ...
  publishedAt: Date,
  portfolioRevision: { type: Number, default: 0, min: 0 },
  ...
  industryMigration: {
    status: { type: String, enum: ['READY', 'IN_PROGRESS', 'REVIEW_REQUIRED', 'COMPLETED', 'FAILED', 'ROLLED_BACK'], default: 'READY' },
    fromIndustry: String,
    toIndustry: String,
    ...
    incompatibleVariants: { type: Number, default: 0 },
    legacyAttributes: { type: Number, default: 0 },
    draftProducts: { type: Number, default: 0 },
    activeCarts: { type: Number, default: 0 },
    ...
    rollbackSnapshot: mongoose.Schema.Types.Mixed,
  ...
  storefrontDesign: {
    ...
    scheduledContentId: { type: String, trim: true, maxlength: 80 },
    scheduledContentFor: Date,
    scheduledContentNote: { type: String, maxlength: 240 },
    scheduledContentBy: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
    scheduledContentStatus: { type: String, enum: ['SCHEDULED', 'PROCESSING', 'FAILED'] },
    scheduledContentAttempts: { type: Number, min: 0 },
...
storeSchema.index({ archivedAt: 1, createdAt: -1 });
storeSchema.index({ customDomain: 1 }, { unique: true, sparse: true });
storeSchema.index({ 'license.status': 1, 'license.endsAt': 1 }, { name: 'license_expiry_due' });
storeSchema.index({ 'license.status': 1, 'license.trialEndsAt': 1 }, { name: 'trial_expiry_due' });
storeSchema.index({ 'storefrontDesign.scheduledContentFor': 1, 'storefrontDesign.scheduledContentStatus': 1 }, { name: 'store_content_release_due' });
